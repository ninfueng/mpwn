...
namespace array
{
	...
	template <typename T, int D1>
	void load_txt(std::string txt_loc, T array[D1])
	{
		std::ifstream FILE;
		FILE.open(txt_loc);
		if(FILE.fail())
		{
			log("ERROR cannot open the file: " << txt_loc);
			exit(1);
		}
		T var = 0;
		int d1_count = 0;
	...
	template <typename T, int D1, int D2>
	void load_txt(std::string txt_loc, T array[D1][D2])
	{
		std::ifstream FILE;
		FILE.open(txt_loc);
		if(FILE.fail())
	...
	template <typename T, int D1, int D2, int D3>
	void load_txt(std::string txt_loc, T array[D1][D2][D3] )
	{
		std::ifstream FILE;
		FILE.open(txt_loc);
		if(FILE.fail())
	...
	template <typename T, int D1, int D2, int D3, int D4>
	void load_txt(std::string txt_loc, T array[D1][D2][D3][D4])
	{
		std::ifstream FILE;
		FILE.open(txt_loc);
		if(FILE.fail())
	...
	void transpose(T x[D1][D2], T y[D2][D1])
	{
